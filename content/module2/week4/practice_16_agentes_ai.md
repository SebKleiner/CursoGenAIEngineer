...
2. Crear un archivo llamado taskweaver_agent.py
...
from dotenv import load_dotenv
import autogen
from autogen import TaskWeaver
...
# Crear el orquestador de tareas
task_manager = TaskWeaver(llm_config={"config_list": config_list})

# Definir el Científico de Datos (DataScientist)
data_scientist = autogen.AssistantAgent(
    name="DataScientist",
    system_message="Eres un científico de datos. Tu tarea es limpiar y analizar los datos.",
    llm_config={"config_list": config_list}
)

# Definir el Analista de Datos (Analyst)
analyst = autogen.AssistantAgent(
    name="Analyst",
    system_message="Eres un analista de datos. Interpreta los resultados y genera un reporte comprensible.",
    llm_config={"config_list": config_list}
)

# Definir el usuario que inicia la tarea
user_proxy = autogen.UserProxyAgent(
    name="user_proxy",
    human_input_mode="NEVER",
    max_consecutive_auto_reply=3,
...
# Conectar los agentes en TaskWeaver
task_manager.connect_agents([data_scientist, analyst])

# Iniciar la tarea
task_manager.start_task(
    user_proxy,
    task="Limpia y analiza un conjunto de datos de ventas, luego genera un reporte con insights clave."
)
...
Links de interés:
- [Documentación](https://microsoft.github.io/TaskWeaver/?WT.mc_id=academic-105485-koreyst)
