...
def render_prompt_evaluation():
    ...
    # Inicializar estado para evaluación de prompts si no existe
    ...
    # Botón para probar todos los prompts
    if st.button("Test All Prompts"):
        ...
        with st.spinner("Testing prompts..."):
            for prompt in st.session_state.prompt_eval['perturbed_prompts']:
                try:
                    # Usar el modelo actual configurado en la aplicación
                    modelo_actual = get_model_instance(st.session_state.config["modelo"])
                    if modelo_actual:
                        # Generar respuesta
                        respuesta = modelo_actual.generate(prompt)
...
def main():
    ...
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["Chat", "Caché", "Métricas", "Historial", "Costos", "Evaluación"])
